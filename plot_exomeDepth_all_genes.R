...
for (i in 1:nsamples) {
  ...
  for (i in 1:nrow(all.exons@annotations)) {
    event.chr=all.exons@annotations[i,]$chromosome
    event.start=all.exons@annotations[i,]$start
    event.end=all.exons@annotations[i,]$end
